    ...
    <Modal open={tagOpen} title="组合用户标签 · 四期" onCancel={() => setTagOpen(false)} onOk={saveTag} okText="保存" destroyOnClose width={820}>
      <Form form={tagForm} layout="vertical" initialValues={{ businessLine: channels[0]?.name, logic: '满足全部条件', rules: [{ field: '用户类型', operator: '等于', value: '' }] }}>
        ...
        <Form.List name="rules">{(fields, { add, remove }) => <>{fields.map((field) => <Form.Item noStyle shouldUpdate key={field.key}>{() => {
          ...
          const options: Record<string, string[]> = { '用户类型': ['测试用户', '正式用户'], '用户体验状态': ['未体验', '体验中', '已体验'], '用户付费状态': ['未付费', '已付费'], '用户订单状态': ['待支付', '已支付', '已取消订阅', '已退费'], '订阅类型': ['周', '月', '年'], '用户标签': tags.map((tag) => tag.name), '有效期状态': ['有效期内', '已过期'] }
