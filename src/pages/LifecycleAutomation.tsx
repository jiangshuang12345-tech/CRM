...
export default function LifecycleAutomation() {
  ...
  const saveTemplate = async () => {
    ...
    setTemplates((items) => [{ id: `tpl_${Date.now()}`, code: `MSG${String(items.length + 1).padStart(4, '0')}`, name: value.name, channel: value.channel, content, tags: value.tags || [], enabled: value.enabled ?? true }, ...items])
