...
type Template = { id: string; code: string; name: string; channel: string; content: string; contentType: 'text' | 'rich'; tags: string[]; enabled: boolean; pushTarget?: string; pushUrl?: string }
...
const seedTemplates: Template[] = [{ id: 'tpl_1', code: 'MSG0001', name: '体验课提醒', channel: 'Push', content: 'Hi {{用户姓名}}，你的体验课已为你准备好，点击即可开始学习。', contentType: 'text', tags: ['体验未完课用户'], enabled: true, pushTarget: '首页' }]
...
export default function LifecycleAutomation() {
  ...
  const openTemplate = (template?: Template) => {
    ...
    templateForm.setFieldsValue(template ?? { channel: 'Push', enabled: true, tags: [], pushTarget: '不跳转' })
    setTemplateOpen(true)
  }
  const saveTemplate = async () => {
    const value = await templateForm.validateFields()
    setTemplates((items) => editingTemplate
      ? items.map((item) => item.id === editingTemplate.id ? { ...item, name: value.name, channel: value.channel, content, contentType, tags: value.tags || [], enabled: value.enabled ?? true, pushTarget: value.channel === 'Push' ? value.pushTarget : undefined, pushUrl: value.channel === 'Push' && value.pushTarget === 'H5页面' ? value.pushUrl : undefined } : item)
      : [{ id: `tpl_${Date.now()}`, code: `MSG${String(items.length + 1).padStart(4, '0')}`, name: value.name, channel: value.channel, content, contentType, tags: value.tags || [], enabled: value.enabled ?? true, pushTarget: value.channel === 'Push' ? value.pushTarget : undefined, pushUrl: value.channel === 'Push' && value.pushTarget === 'H5页面' ? value.pushUrl : undefined }, ...items])
  ...
  return <div>
    ...
    <Modal open={templateOpen} title={`${editingTemplate ? '编辑' : '新建'}消息模板 · 四期`} onCancel={() => { setTemplateOpen(false); setEditingTemplate(null) }} onOk={saveTemplate} okText="保存" destroyOnClose width={760}><Form form={templateForm} layout="vertical"><Form.Item name="name" label="模板名称" rules={[{ required: true, message: '请输入模板名称' }]}><Input /></Form.Item><Form.Item name="channel" label="发送通道" rules={[{ required: true }]}><Select disabled={!!editingTemplate} options={['Push', 'Email', '短信'].map((value) => ({ value }))} /></Form.Item><Form.Item noStyle shouldUpdate={(previous, current) => previous.channel !== current.channel || previous.pushTarget !== current.pushTarget}>{({ getFieldValue }) => getFieldValue('channel') === 'Push' && <><Form.Item name="pushTarget" label="点击 Push 后跳转" rules={[{ required: true, message: '请选择跳转页面' }]}><Select placeholder="请选择" options={['不跳转', '首页', '课表', '约课', '完课', '我的', 'H5页面'].map((value) => ({ value, label: value === 'H5页面' ? 'H5页面（需要填跳转地址）' : value }))} /></Form.Item>{getFieldValue('pushTarget') === 'H5页面' && <Form.Item name="pushUrl" label="H5 跳转地址" rules={[{ required: true, type: 'url', message: '请输入有效的 H5 链接' }]}><Input placeholder="https://example.com/page" /></Form.Item>}</>}</Form.Item><Form.Item label="消息内容" required><Radio.Group value={contentType} onChange={(e) => setContentType(e.target.value)} optionType="button" options={[{ value: 'text', label: '纯文本' }, { value: 'rich', label: '富文本' }]} style={{ marginBottom: 10 }} />{contentType === 'text' ? <Input.TextArea value={content} onChange={(e) => setContent(e.target.value)} rows={5} maxLength={1000} placeholder="填写消息内容" /> : <div style={{ border: '1px solid #d9d9d9', borderRadius: 6 }}><div style={{ padding: '7px 8px', borderBottom: '1px solid #d9d9d9', background: '#fafafa' }}><Space size={2} wrap><Select size="small" defaultValue="3" style={{ width: 82 }} onChange={(v) => formatRichText('fontSize', v)} options={[['2','12px'],['3','16px'],['4','18px'],['5','22px']].map(([value,label])=>({value,label}))}/>{[['bold','B'],['italic','I'],['underline','U'],['strikeThrough','S'],['justifyLeft','≡'],['justifyCenter','≣'],['justifyRight','☰'],['insertUnorderedList','• 列表'],['insertOrderedList','1. 列表']].map(([command,label]) => <Button key={command} size="small" type="text" onMouseDown={(e) => e.preventDefault()} onClick={() => formatRichText(command)} style={{ fontWeight: command === 'bold' ? 700 : undefined, fontStyle: command === 'italic' ? 'italic' : undefined }}>{label}</Button>)}<ColorPicker size="small" onChangeComplete={(color) => formatRichText('foreColor', color.toHexString())}><Button size="small" type="text" style={{ color: '#1677ff', textDecoration: 'underline' }}>A</Button></ColorPicker><Button size="small" onMouseDown={(e) => e.preventDefault()} onClick={() => formatRichText('createLink', window.prompt('输入链接地址') || '')}>链接</Button><Button size="small" onClick={() => { setPendingImage(''); setImageUploadOpen(true) }}>图片</Button></Space></div><div contentEditable suppressContentEditableWarning onInput={(e) => setContent(e.currentTarget.innerHTML)} dangerouslySetInnerHTML={{ __html: content }} style={{ minHeight: 220, padding: 12, outline: 'none' }} /></div>}<Text type="secondary">点击加入预定义变量：</Text><div style={{ marginTop: 8 }}><Space wrap>{variables.map((v) => <Button size="small" key={v} onClick={() => addVariable(v)}>{`{{${v}}}`}</Button>)}</Space></div></Form.Item><Form.Item name="tags" label="选择用户标签"><Select mode="multiple" options={tags.map((tag) => ({ value: tag.name }))} /></Form.Item><Form.Item name="enabled" label="是否启用" valuePropName="checked"><Switch /></Form.Item></Form></Modal>
