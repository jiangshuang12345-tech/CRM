...
type Template = { id: string; code: string; name: string; channel: string; content: string; tags: string[]; enabled: boolean }
...
const seedTemplates: Template[] = [{ id: 'tpl_1', code: 'MSG0001', name: '体验课提醒', channel: 'Push', content: 'Hi {{用户姓名}}，你的体验课已为你准备好，点击即可开始学习。', tags: ['体验未完课用户'], enabled: true }]
...
export default function LifecycleAutomation() {
  ...
  const saveTemplate = async () => {
    ...
    setTemplates((items) => editingTemplate
      ? items.map((item) => item.id === editingTemplate.id ? { ...item, name: value.name, channel: value.channel, content, tags: value.tags || [], enabled: value.enabled ?? true } : item)
      : [{ id: `tpl_${Date.now()}`, code: `MSG${String(items.length + 1).padStart(4, '0')}`, name: value.name, channel: value.channel, content, tags: value.tags || [], enabled: value.enabled ?? true }, ...items])
